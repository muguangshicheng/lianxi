...
class Site extends IController
{
	...
	function index()
	{
	    ...
	    if (isset($_GET['code'])){
	        $code = $_GET['code'];
	       // echo $code;
            $url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=[YOUR_APP_ID]&client_secret=[YOUR_APP_Key]&code=[The_AUTHORIZATION_CODE]&state=[The_CLIENT_STATE]&redirect_uri=[YOUR_REDIRECT_URI]";
	    ...
	    $ip='118.25.91.205';
	    $ak='uQOh5EhXQ0OFL3C87nHGVkU93AW0Gzr9';
	    $url="http://api.map.baidu.com/location/ip";
	    $str = file_get_contents($url);
	    $data = json_decode($str,true);
	    $city = $data['content']['address_detall']['city'];
	    $this->city = $city;
